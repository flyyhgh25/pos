    ...
    <div class="p-8">
        <div class="mb-4 flex items-center justify-between">
            <div>
                ...
                <p class="text-lg font-semibold text-slate-800">{{ $pembelian->no_transaksi }}</p>
            </div>
            <div class="text-right text-sm text-slate-500">
                <p>{{ $pembelian->tanggal->format('d M Y') }}</p>
                <p>Supplier: {{ $pembelian->supplier->nama }}</p>
                <p>Dicatat oleh: {{ $pembelian->user->name }}</p>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                ...
                <tfoot>
                    <tr class="bg-slate-50 font-medium">
                        ...
                        <td class="px-4 py-2.5">Rp {{ number_format($pembelian->total_harga, 0, ',', '.') }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
